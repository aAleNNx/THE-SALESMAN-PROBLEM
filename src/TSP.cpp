...
path_t StageState::get_path() {
    ...
    std::vector<std::size_t> next_node(N, N);

    for (const auto& edge : unsorted_path_) {
        next_node[edge.row] = edge.col;
    }
...
std::vector<cost_t> CostMatrix::get_min_values_in_rows() const {
    ...
    cost_t current_min = -1;
    for (std::size_t r = 0; r < matrix_.size(); r++){
        for (std::size_t c = 0; c < matrix_.size(); c++){
            if (current_min == -1){
                current_min = matrix_[r][c];
            }
            else if (matrix_[r][c] <= current_min){
                current_min = matrix_[r][c];
            }
        }
        min_values.push_back(current_min);
        current_min = -1;
    }
    return min_values;
}
